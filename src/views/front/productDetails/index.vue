        ...
        <Col class-name="center">
            <Row justify="center" class-name="good">
                <Col class-name="good-img" span="12">
                  <img :src=" agencyStr + '/file' + dataDetails.data.data_pic"  :alt="dataDetails.data_name">  
                </Col>
                <Col span="12">
                    <Row class-name="data-base-info">
                        <Col span="24">
                            ...
                            <div class="price">{{ dataDetails.data.data_per_price }}&nbsp 元</div>
                            <Divider/>
                        </Col>
                    </Row>
                    <!-- 选择规格表单 -->
                    <Row class-name="spec">
                        <Col span="24">
                            ...
                            <Form :model="AppliForm" :label-width="100">
                                <!-- 文本结构化表单 -->
                                <FormItem v-if="['文本', '结构化'].indexOf(dataDetails.data.data_type) > -1" :label="
                                    dataDetails.data.data_type === '文本'?'文本的条数':
                                    (dataDetails.data.data_type === '结构化'?'数据的条数':
                                    '数据的条数')
                                ">
                                    <InputNumber v-model="AppliForm.item[0].num" :min="0" :max="dataDetails.data.data_attr.num" controls-outside></InputNumber>
                                    <Button class-name="max" type="text" shape="circle" 
                                            @click="AppliForm.item[0].num=dataDetails.data.data_attr.num">最大</Button>
                                </FormItem>
                                <!-- 图片表单 -->
                                <FormItem v-if="dataDetails.data.data_type==='图片'" label="清晰度">
                                    <Row class-name="num" v-for="(item,index) in dataDetails.data.data_attr.num" :key="index">
                                        <Col span="3">{{ item.dpi }}dpi</Col>
                                        <Col>
                                            <InputNumber v-if="AppliForm.item[index]" 
                                                v-model="AppliForm.item[index].num" controls-outside 
                                                :min="0" :max="item.num"/> 张
                                        </Col>
                                        <Col><Button class-name="max" type="text" shape="circle" 
                                            @click="AppliForm.item[index].num=item.num">最大</Button></Col>
                                    </Row>
                                </FormItem>
                                <!-- 结构化表单 -->
                                <FormItem v-if="dataDetails.data.data_type==='结构化' 
                                                && dataDetails.data.data_preview 
                                                && dataDetails.data.data_preview.columns" 
                                    label="选择属性">
                                    <CheckboxGroup  v-model="AppliForm.item.item">
                                        <Checkbox  v-for="(item,index) in dataDetails.data.data_preview.columns" :key="item.key" :label="item.title"></Checkbox>
                                    </CheckboxGroup>
                                </FormItem>
                                <!-- 视频音频表单 -->
                                <FormItem v-if="['视频','音频'].indexOf(dataDetails.data.data_type) > -1" :label="dataDetails.data.data_type==='视频'?'清晰度':'音质'">
                                    <template v-for="(item,index) in dataDetails.data.data_attr.num"  :key="index">
                                        <Row class-name="num" v-if="item.num">
                                            <Col span="3">{{ item.title }}</Col>
                                            <Col>
                                                <InputNumber v-if="AppliForm.item[index]" 
                                                    v-model="AppliForm.item[index].num" controls-outside 
                                                    :min="0" :max="item.num"/> 个
                                            </Col>
                                            <Col><Button class-name="max" type="text" shape="circle" 
                                                @click="AppliForm.item[index].num=item.num">最大</Button></Col>
                                        </Row>
                    ...
                    <Row class-name="sum">
                        <Col span="24">
                            <Row>
                                <Col class-name="sub-title"  span="24"><div>购买的物品</div></Col>
                            </Row>
                            <template v-for="(item,index) in AppliForm.item" :key="index">
                                <Row class-name="item" v-if="item.num > 0">
                                    ...
                                    <Col class-name="item-amount" span="6">共{{ dataDetails.data.data_per_price * item.num }}元</Col>
                                </Row>
                            </template>
                            <Row>
                                <Col span="24">
            ...
            <Row justify="space-between">
                <Col span="11" class-name="details">
                    ...
                    <Row class-name="table-row">
                        ...
                        <Col class-name="table-cell table-right" span="18"><div>{{ dataDetails.data.data_name }}</div></Col> 
                    </Row>
                    <Row class-name="table-row">
                        <Col class-name="table-cell" span="6">商家昵称</Col>
                        <Col class-name="table-cell table-right" span="18"><div>{{ dataDetails.seller.seller_name }}</div></Col> 
                    </Row>
                    <Row class-name="table-row">
                        <Col class-name="table-cell" span="6">数据介绍</Col>
                        <Col class-name="table-cell table-right" span="18"><div>{{ dataDetails.data.data_intro }}</div></Col> 
                    </Row>
                    <Row class-name="table-row">
                        <Col class-name="table-cell" span="6">数据类型</Col>
                        <Col class-name="table-cell table-right" span="18"><div>{{ dataDetails.data.data_type }}</div></Col> 
                    </Row>
                    <Row class-name="table-row">
                        <Col class-name="table-cell" span="6">产品类别</Col>
                        <Col class-name="table-cell table-right" span="18"><div>{{ dataDetails.data.data_use }}</div></Col> 
                    </Row>
                    <Row class-name="table-row" v-if="dataDetails.data.data_type === '文本'">
                        <Col class-name="table-cell" span="6">平均长度</Col>
                        <Col class-name="table-cell table-right" span="18"><div>
                    ...
                    <Row class-name="table-row" v-if="dataDetails.data.data_type === '结构化'">
                    ...
                    <Row class-name="table-row" v-if="dataDetails.data.data_type === '结构化'">
                        ...
                        <Col class-name="table-cell table-right" span="18"><div>
                            {{ dataDetails.data.data_attr?
                                dataDetails.data.data_preview.columns.map(item => item.title).join('、'): '' 
                            }}个
                        </div></Col> 
                    </Row>
                    <Row class-name="table-row" v-if="dataDetails.data.data_type === '结构化'">
                    ...
                    <Row class-name="table-row" v-if="dataDetails.data.data_type === '图片'">
                    ...
                    <Row class-name="table-row" v-if="dataDetails.data.data_type === '图片'">
                        ...
                        <Col class-name="table-cell table-right" span="18"><div>
                            <template v-if="dataDetails.data.data_attr && dataDetails.data.data_attr.num"
                                    v-for="(item, index) in dataDetails.data.data_attr.num">
                                <Row>
                                    <Col span="24">大小{{ item.width }}×{{ item.height }}px，{{ item.dpi }}dpi，{{ item.num }}张</Col>
                                </Row>
                            </template>
                        </div></Col> 
                    </Row>
                    <Row class-name="table-row" v-if="dataDetails.data.data_type === '视频'">
                    ...
                    <Row class-name="table-row" v-if="dataDetails.data.data_type === '音频'">
                    ...
                    <Row class-name="table-row">
                        ...
                        <Col class-name="table-cell table-right table-bottom" span="18"><div>{{ dataDetails.data.data_field }}</div></Col> 
                    </Row>                   
                </Col>
                    ...
                    <Row class-name="table-row" justify="center">
                        <Col span="24">
                            <Row v-if="dataDetails.data.data_type === '图片'">
                                <Swiper v-if="dataDetails.data.data_preview.sample.length > 2"
                                    class="mySwiper"
                                    :freeModel="true"
                                    :loop="(dataDetails.data.data_preview.sample.length > 4?true:false)"
                                    :speed="5000"
                                    :slidesPerView= "2"
                                    :autoplay=" {
                                        delay: 0,  
                                    ...
                                    :modules="modules"
                                >
                                    <SwiperSlide v-for="(item, index) in dataDetails.data.data_preview.sample" :key="index">
                                        <img :src="agencyStr + '/file' + item"  :alt="dataDetails.data_name">
                                    </SwiperSlide>
                                </Swiper>
                                <div v-else >
                                    <div v-for="(item, index) in dataDetails.data.data_preview.sample" :key="index">
                                        <img :src="agencyStr + '/file' + item"  :alt="dataDetails.data_name">
                                    </div>
                                </div>
                            </Row>
                                ...
                                <Col span="24">
                                    <Row>
                                        <Col span="24">
                                            ...
                                            <Tag class="audio-tag" v-for="(item, index) in audioList" 
                                                :key="index" type="border" 
                                                @click="handleTagClick(index, $refs.audioPlayer)"  
                                                @on-close="handleTagClose(index), handleRemove(index, '数据样例')" 
                            ...
                            <Row v-if="dataDetails.data.data_type === '视频'">
                                <Col span="24">
                                    <div class="player-container">
                                        <video-player
                                            v-for="(item, index) in dataDetails.data.data_preview.sample"
                                            class="video-palyer"
                                            width="300px"
                                            muted
                                            controls
                            ...
                            <Row v-if="['文本', '结构化'].indexOf(dataDetails.data.data_type) > -1">
                                <Table stripe  style="background-color: #f5f5f5;"  size="small" height="400" :columns="dataDetails.data.data_preview.columns" :data="dataDetails.data.data_preview.data" border />
                            </Row>
                        </Col>
                    </Row>
                </Col>
    ...
    getDataDetail(params).then((res) => {
        if(res.data.code === 0) {
            ...
            if (dataDetails.value.data.data_type === "音频"){
                setAudioList(dataDetails.value.data.data_preview.sample.map((item) => {
